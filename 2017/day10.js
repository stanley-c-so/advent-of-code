...
function knotHash (part, inputStr, DEBUG = false) {
  
  // CONSTANTS
  const SKIP_LENGTHS = part === 1 ? inputStr.split(',').map(n => +n)                        // PART 1: TREAT INPUT STRING AS NUMBERS
                                  : inputStr.split('').map(c => c.charCodeAt(0))            // PART 2: TREAT INPUT STRING AS BYTES
                                      .concat([ 17, 31, 73, 47, 23 ]);                      // (problem tells you to add these nums to the end)

  const LIST_SIZE = part === 1 && DEBUG ? 5 : 256;                                          // LIST_SIZE is 256 except for part 1 sample

  const NUM_ROUNDS = part === 1 ? 1                                                         // PART 1: DO 1 ROUND OF ITERATING THROUGH SKIP LENGTHS
                                : 64;                                                       // PART 2: DO 64 ROUNDS
  
  // DATA STRUCTURES
  const LIST = [ ...Array(LIST_SIZE).keys() ];
  
  // UTILITY FUNCTION - GIVEN AN INDEX THAT MAY BE OUT OF BOUNDS, USE MOD TO GET TRUE INDEX
  const getIdx = idx => (LIST_SIZE + idx) % LIST_SIZE;                                      // first add LIST_SIZE in case too low
                                                                                            // then at any rate, mod LIST_SIZE in case too high
  ...
  // ANALYZE
  if (part === 1) {                                                                         // PART 1: RETURN PRODUCT OF FIRST TWO NUMBERS OF LIST

    return LIST[0] * LIST[1];

  } else {                                                                                  // PART 2: CONVERT LIST TO DENSE HASH, THEN CONVERT TO HEX
    ...
    return denseHash.map(block => block.toString(BLOCK_SIZE).padStart(2, '0')).join('');    // don't forget to pad each hex representation to 2 digits
