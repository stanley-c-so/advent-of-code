...
function foldingPaper (part, inputStr) {

  // PARSE INPUT
  const [dotsChunk, foldsChunk] = inputStr.split('\n\n');
  const dots = dotsChunk.split('\n').map(coords => coords.split(',').map(n => +n));
  const folds = foldsChunk.split('\n').map(line => {
    const [instruction, val] = line.split('=');
    const axis = instruction.split('fold along ')[1];
    return [axis, +val];
  });
...
expected = undefined;     // the answer is determined visually from the readout. the answer is RLBCJGLU
test(func, input, expected, testNum, lowestTest, highestTest);