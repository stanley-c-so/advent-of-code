...
function diracDice (part, inputStr) {
  ...
  if (part === 1) {                                                                                   // PART 1: use deterministic die only
    ...
    while (true) {
      ...
      // roll the deterministic die 3 times...
      let roll = 0;
      for (let k = 0; k < 3; ++k) {
        roll += rollDeteriministicDie();
      }
      numTimesRolled += 3;

      // ...then calculate the new game state
      p1Position = p1Turn ? modWithoutZero(p1Position + roll, 10) : p1Position;
  ...
  } else {                                                                                          // PART 2: use the quantum die

    // THE WAY I SHORTCUT ALL THE POSSIBILITIES IS TO REALIZE THAT THERE IS A FREQUENCY DISTRIBUTION FOR
    // THE OUTCOME OF THE 3 DICE ROLLS THAT CONSTITUTE A SINGLE TURN:
    const diceOutcomes = [];
    for (let i = 1; i <= 3; ++i) {
      for (let j = 1; j <= 3; ++j) {
        for (let k = 1; k <= 3; ++k) {
          diceOutcomes.push(i + j + k);
        }
      }
    }
    const diceOutcomesDict = {}
    for (const outcome of diceOutcomes) {
      diceOutcomesDict[outcome] = diceOutcomesDict[outcome] + 1 || 1;
    }
    // FOR REFERENCE:
    // {
    //   3: 1,
    ...
    const go = simulate(p1Turn, p1Position, p2Position, p1Score, p2Score);
...
const sampleInput = `Player 1 starting position: 4
Player 2 starting position: 8`;

const actualInput = `Player 1 starting position: 6
