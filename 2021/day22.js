...
function overlappingBoxes (part, inputStr) {

  // PARSE DATA AND CONVERT EVERY LINE INTO A BOX REPRESENTING A REGION
  const inputArr = inputStr.split('\n').map(line => {
    const split = line.split(' ');
    return {
      mode: split[0],
      data: split[1].split(',').map(axisData => axisData.split('=')[1].split('..').map(n => +n))
    }
  });

  if (part === 1) {                                                                                 // PART 1: only count region from -50..50 for each axis

    // BRUTE FORCE: SIMPLY ITERATE THROUGH EVERY POINT IN THE GIVEN REGION AND TRACK ON/OFF STATUS WITH EACH LINE OF INPUT
    const onPoints = new Set();
    for (const line of inputArr) {
      const [[x1, x2], [y1, y2], [z1, z2]] = line.data;
      for (let x = -50; x <= 50; ++x) {
        for (let y = -50; y <= 50; ++y) {
          for (let z = -50; z <= 50; ++z) {
            if (x1 <= x && x <= x2 && y1 <= y && y <= y2 && z1 <= z && z <= z2) {                   // if point overlaps with region from line of input...
              const serializedPoint = `${x},${y},${z}`;
              if (line.mode === 'on') onPoints.add(serializedPoint);                                // ...switch it on if the mode is 'on'...
              else onPoints.delete(serializedPoint);                                                // ...or switch it off if the mode is 'off'
            }
          }
        }
      }
  ...
  } else {                                                                                          // PART 2: consider the entire space

    // UTILITY FUNCTIONS

    const boxToSerial = ([[x1, x2], [y1, y2], [z1, z2]]) => `[[${x1},${x2}],[${y1},${y2}],[${z1},${z2}]]`;

    const serialToBox = str => str.slice(2, str.length - 2).split('],[').map(rangeStr => rangeStr.split(',').map(n => +n));

    function overlap(boxA, boxB) {
      const [[aX1, aX2], [aY1, aY2], [aZ1, aZ2]] = boxA;
      const [[bX1, bX2], [bY1, bY2], [bZ1, bZ2]] = boxB;
      let xOverlap = aX1 < bX1 ? bX1 <= aX2 : aX1 <= bX2;
      let yOverlap = aY1 < bY1 ? bY1 <= aY2 : aY1 <= bY2;
      let zOverlap = aZ1 < bZ1 ? bZ1 <= aZ2 : aZ1 <= bZ2;
      return xOverlap && yOverlap && zOverlap;
    }

    // HELPER FUNCTION - EXISTING BOX IS TARGET; INCOMING BOX IS SPLITTER. SPLIT TARGET BOX IN REGIONS OF OVERLAP

    function splitBox(targetBox, splitterBox) {

      if (!overlap(targetBox, splitterBox)) return [ targetBox ];                                   // if no overlap, no need to split targetBox

      const [ [tX1, tX2], [tY1, tY2], [tZ1, tZ2] ] = targetBox;
      const [ [sX1, sX2], [sY1, sY2], [sZ1, sZ2] ] = splitterBox;
      
      const tX1SmallerThanSX1 = tX1 < sX1;
      const tY1SmallerThanSY1 = tY1 < sY1;
      const tZ1SmallerThanSZ1 = tZ1 < sZ1;
      const tX2GreaterThanSX2 = tX2 > sX2;
      const tY2GreaterThanSY2 = tY2 > sY2;
      const tZ2GreaterThanSZ2 = tZ2 > sZ2;

      // CONSIDER ALL 26 POTENTIAL SUB-REGIONS WHEN splitterBox SPLITS targetBox, AND GATHER THEM IN boxes ARRAY

      const boxes = [];

      // tX1 < sX1, tY2 > sY2, tZ2 > sZ2
      if (tX1SmallerThanSX1 && tY2GreaterThanSY2 && tZ2GreaterThanSZ2) {
        boxes.push([ [tX1, sX1 - 1], [sY2 + 1, tY2], [sZ2 + 1, tZ2] ]);
      }
      // _________, tY2 > sY2, tZ2 > sZ2
      if (true && tY2GreaterThanSY2 && tZ2GreaterThanSZ2) {
        boxes.push([ [Math.max(tX1, sX1), Math.min(sX2, tX2)], [sY2 + 1, tY2], [sZ2 + 1, tZ2] ]);
      }
      // tX2 > sX2, tY2 > sY2, tZ2 > sZ2
      if (tX2GreaterThanSX2 && tY2GreaterThanSY2 && tZ2GreaterThanSZ2) {
        boxes.push([ [sX2 + 1, tX2], [sY2 + 1, tY2], [sZ2 + 1, tZ2] ]);
      }
      // tX1 < sX1, tY2 > sY2, _________
      if (tX1SmallerThanSX1 && tY2GreaterThanSY2 && true) {
        boxes.push([ [tX1, sX1 - 1], [sY2 + 1, tY2], [Math.max(tZ1, sZ1), Math.min(sZ2, tZ2)] ]);
      }
      // _________, tY2 > sY2, _________
      if (true && tY2GreaterThanSY2 && true) {
        boxes.push([ [Math.max(tX1, sX1), Math.min(sX2, tX2)], [sY2 + 1, tY2], [Math.max(tZ1, sZ1), Math.min(sZ2, tZ2)] ]);
      }
      // tX2 > sX2, tY2 > sY2, _________
      if (tX2GreaterThanSX2 && tY2GreaterThanSY2 && true) {
        boxes.push([ [sX2 + 1, tX2], [sY2 + 1, tY2], [Math.max(tZ1, sZ1), Math.min(sZ2, tZ2)] ]);
      }
      // tX1 < sX1, tY2 > sY2, tZ1 < sZ1
      if (tX1SmallerThanSX1 && tY2GreaterThanSY2 && tZ1SmallerThanSZ1) {
        boxes.push([ [tX1, sX1 - 1], [sY2 + 1, tY2], [tZ1, sZ1 - 1] ]);
      }
      // _________, tY2 > sY2, tZ1 < sZ1
      if (true && tY2GreaterThanSY2 && tZ1SmallerThanSZ1) {
        boxes.push([ [Math.max(tX1, sX1), Math.min(sX2, tX2)], [sY2 + 1, tY2], [tZ1, sZ1 - 1] ]);
      }
      // tX2 > sX2, tY2 > sY2, tZ1 < sZ1
      if (tX2GreaterThanSX2 && tY2GreaterThanSY2 && tZ1SmallerThanSZ1) {
        boxes.push([ [sX2 + 1, tX2], [sY2 + 1, tY2], [tZ1, sZ1 - 1] ]);
      }

      // tX1 < sX1, _________, tZ2 > sZ2
      if (tX1SmallerThanSX1 && true && tZ2GreaterThanSZ2) {
        boxes.push([ [tX1, sX1 - 1], [Math.max(tY1, sY1), Math.min(sY2, tY2)], [sZ2 + 1, tZ2] ]);
      }
      // _________, _________, tZ2 > sZ2
      if (true && true && tZ2GreaterThanSZ2) {
        boxes.push([ [Math.max(tX1, sX1), Math.min(sX2, tX2)], [Math.max(tY1, sY1), Math.min(sY2, tY2)], [sZ2 + 1, tZ2] ]);
      }
      // tX2 > sX2, _________, tZ2 > sZ2
      if (tX2GreaterThanSX2 && true && tZ2GreaterThanSZ2) {
        boxes.push([ [sX2 + 1, tX2], [Math.max(tY1, sY1), Math.min(sY2, tY2)], [sZ2 + 1, tZ2] ]);
      }
      // tX1 < sX1, _________, _________
      if (tX1SmallerThanSX1 && true && true) {
        boxes.push([ [tX1, sX1 - 1], [Math.max(tY1, sY1), Math.min(sY2, tY2)], [Math.max(tZ1, sZ1), Math.min(sZ2, tZ2)] ]);
      }

      // _________, _________, _________
      // SKIP - THIS REGION REPRESENTS THE SPLITTER BOX ITSELF!

      // tX2 > sX2, _________, _________
      if (tX2GreaterThanSX2 && true && true) {
        boxes.push([ [sX2 + 1, tX2], [Math.max(tY1, sY1), Math.min(sY2, tY2)], [Math.max(tZ1, sZ1), Math.min(sZ2, tZ2)] ]);
      }
      // tX1 < sX1, _________, tZ1 < sZ1
      if (tX1SmallerThanSX1 && true && tZ1SmallerThanSZ1) {
        boxes.push([ [tX1, sX1 - 1], [Math.max(tY1, sY1), Math.min(sY2, tY2)], [tZ1, sZ1 - 1] ]);
      }
      // _________, _________, tZ1 < sZ1
      if (true && true && tZ1SmallerThanSZ1) {
        boxes.push([ [Math.max(tX1, sX1), Math.min(sX2, tX2)], [Math.max(tY1, sY1), Math.min(sY2, tY2)], [tZ1, sZ1 - 1] ]);
      }
      // tX2 > sX2, _________, tZ1 < sZ1
      if (tX2GreaterThanSX2 && true && tZ1SmallerThanSZ1) {
        boxes.push([ [sX2 + 1, tX2], [Math.max(tY1, sY1), Math.min(sY2, tY2)], [tZ1, sZ1 - 1] ]);
      }

      // tX1 < sX1, tY1 < sY1, tZ2 > sZ2
      if (tX1SmallerThanSX1 && tY1SmallerThanSY1 && tZ2GreaterThanSZ2) {
        boxes.push([ [tX1, sX1 - 1], [tY1, sY1 - 1], [sZ2 + 1, tZ2] ]);
      }
      // _________, tY1 < sY1, tZ2 > sZ2
      if (true && tY1SmallerThanSY1 && tZ2GreaterThanSZ2) {
        boxes.push([ [Math.max(tX1, sX1), Math.min(sX2, tX2)], [tY1, sY1 - 1], [sZ2 + 1, tZ2] ]);
      }
      // tX2 > sX2, tY1 < sY1, tZ2 > sZ2
      if (tX2GreaterThanSX2 && tY1SmallerThanSY1 && tZ2GreaterThanSZ2) {
        boxes.push([ [sX2 + 1, tX2], [tY1, sY1 - 1], [sZ2 + 1, tZ2] ]);
      }
      // tX1 < sX1, tY1 < sY1, _________
      if (tX1SmallerThanSX1 && tY1SmallerThanSY1 && true) {
        boxes.push([ [tX1, sX1 - 1], [tY1, sY1 - 1], [Math.max(tZ1, sZ1), Math.min(sZ2, tZ2)] ]);
      }
      // _________, tY1 < sY1, _________
      if (true && tY1SmallerThanSY1 && true) {
        boxes.push([ [Math.max(tX1, sX1), Math.min(sX2, tX2)], [tY1, sY1 - 1], [Math.max(tZ1, sZ1), Math.min(sZ2, tZ2)] ]);
      }
      // tX2 > sX2, tY1 < sY1, _________
      if (tX2GreaterThanSX2 && tY1SmallerThanSY1 && true) {
        boxes.push([ [sX2 + 1, tX2], [tY1, sY1 - 1], [Math.max(tZ1, sZ1), Math.min(sZ2, tZ2)] ]);
      }
      // tX1 < sX1, tY1 < sY1, tZ1 < sZ1
      if (tX1SmallerThanSX1 && tY1SmallerThanSY1 && tZ1SmallerThanSZ1) {
        boxes.push([ [tX1, sX1 - 1], [tY1, sY1 - 1], [tZ1, sZ1 - 1] ]);
      }
      // _________, tY1 < sY1, tZ1 < sZ1
      if (true && tY1SmallerThanSY1 && tZ1SmallerThanSZ1) {
        boxes.push([ [Math.max(tX1, sX1), Math.min(sX2, tX2)], [tY1, sY1 - 1], [tZ1, sZ1 - 1] ]);
      }
      // tX2 > sX2, tY1 < sY1, tZ1 < sZ1
      if (tX2GreaterThanSX2 && tY1SmallerThanSY1 && tZ1SmallerThanSZ1) {
        boxes.push([ [sX2 + 1, tX2], [tY1, sY1 - 1], [tZ1, sZ1 - 1] ]);
      }

      return boxes;
    }

    // MAIN FUNCTION: FOR EVERY LINE OF INPUT, CHECK THE NEW REGION AGAINST EXISTING ONES, AND SPLIT THE OLD ONES AS NEEDED.
    // THE LOOP INVARIANT IS THAT EVERY REGION INSIDE THE SET IS MUTUALLY EXCLUSIVE (NO OVERLAPS) WITH EVERY OTHER
    const boxes = new Set();
    for (const line of inputArr) {
      const newBox = line.data;
      for (const boxStr of [...boxes]) {
        boxes.delete(boxStr);
        const splitResults = splitBox(serialToBox(boxStr), newBox).map(box => boxToSerial(box));
        for (const result of splitResults) boxes.add(result);
      }
      if (line.mode === 'on') boxes.add(boxToSerial(newBox));
    }

    // WITH THE FINAL SET OF REGIONS, SIMPLY ADD THE VOLUME OF EACH ONE TO THE RUNNING TOTAL
    let count = 0;
    for (const boxStr of boxes) {
      const box = serialToBox(boxStr);
      count += (box[0][1] - box[0][0] + 1) * (box[1][1] - box[1][0] + 1) * (box[2][1] - box[2][0] + 1);
    }
    return count;
...
const testNum = [1];
let input, expected;
const func = overlappingBoxes;
const sortedFunc = (...args) => func(...args).sort();                   // used when the order of the output does not matter
const modFunc = (...args) => func(...args) % 1000000007;                // used when the output is very large
const lowestTest = 0 || 0;
const highestTest = 0 || Infinity;

const sampleInput1 = `on x=10..12,y=10..12,z=10..12
on x=11..13,y=11..13,z=11..13
off x=9..11,y=9..11,z=9..11
on x=10..10,y=10..10,z=10..10`;

const sampleInput2 = `on x=-20..26,y=-36..17,z=-47..7
on x=-20..33,y=-21..23,z=-26..28
on x=-22..28,y=-29..23,z=-38..16
on x=-46..7,y=-6..46,z=-50..-1
on x=-49..1,y=-3..46,z=-24..28
...
const sampleInput3 = `on x=-5..47,y=-31..22,z=-19..33
on x=-44..5,y=-27..21,z=-14..35
on x=-49..-1,y=-11..42,z=-10..38
on x=-20..34,y=-40..6,z=-44..1
off x=26..39,y=40..50,z=-2..11
...
const actualInput = `on x=-5..46,y=-32..20,z=-18..26
