...
function CRT (part, inputStr, DEBUG = false) {
  ...
  } else {                                                        // PART 2: DRAW CRT IMAGE BASED ON SPRITE POSITION RELATIVE TO DRAW PIXEL
    
    const H = 6, W = 40;
    const CRT = Array.from({length: H}, () => Array(W));
    let row = 0, col = 0;                                         // coords of pixel being drawn

    function renderCRT() {
      CRT[row][col] = (Math.abs(col - x) <= 1) ? '#' : '.';       // '#' if draw pixel is within 1 position of middle of sprite (x), else '.'
      ++col;                                                      // advance draw pixel
      if (col === W) {                                            // move to next row when col exceeds width
        ++row;
        col = 0;
      }
    }
