...
function getNumAtPositionInInfiniteGrid (part, inputStr, DEBUG = false) {
  const split = inputStr.split(' ');
  const TARGET_ROW = +(split[split.indexOf('row') + 1].slice(0, -1));
  const TARGET_COL = +(split[split.indexOf('column') + 1].slice(0, -1));

  const gauss = n => n * (n + 1) / 2;
  const NUM_OPERATIONS = gauss(TARGET_ROW + TARGET_COL - 1 - 1)             // the diagonal you are on is the (TARGET_ROW + TARGET_COL - 1)th diagonal.
                                                                            // (from target coords, if you keep moving down-left, you see which diagonal
                                                                            // you are on when you reach column 1.)
                                                                            // let's call that the dth diagonal. from there, the (d-1)th triangular number
                                                                            // is how many coordinates come before your current position on column 1. use
                                                                            // gauss' formula to figure that part out.
                          + TARGET_COL - 1;                                 // then it takes TARGET_COL more numbers to get to the target coords.
                                                                            // however, since you start at the top left, it doesn't take a move to get there,
                                                                            // so subtract 1 again.

  if (DISPLAY_EXTRA_INFO) {
    console.log(`(we need to perform ${NUM_OPERATIONS} operations)`);
  }

  const TIME_AT_START = Date.now();
  console.log('RUNNING ANALYSIS (PLEASE WAIT)...');
  
  // ANALYZE - JUST BRUTE FORCE IT. NO TRICKS. IT'S FAST!
  let n = 20151125;
...
const testNum = [1];
let input, expected;
const func = getNumAtPositionInInfiniteGrid;
