...
function analyzeBags (part, inputStr) {
  const inputArr = inputStr.split('\n');

  // STEP 1: FIRST, CREATE A DICTIONARY BASED ON INPUT RULES, SO WE CAN LOOK UP WHAT A GIVEN BAG COLOR MUST CONTAIN

  const dict = {};
  for (const rule of inputArr) {
    const [container, contents] = rule.split(' bags contain ');                   // e.g. 'light red bags contain 1 bright white bag, 2 muted yellow bags.' --> ['light red', '1 bright ... bags.']
    if (!(container in dict)) dict[container] = {};
    for (const content of contents.slice(0, contents.length - 1).split(', ')) {   // e.g. ['1 bright white bag', '2 muted yellow bags'] (we got rid of the period at the end before splitting)
      const split = content.split(' ');                                           // e.g. ['1', 'bright', 'white', 'bag']
      if (contents !== 'no other bags.') {                                        // (remember, it is possible that the rule was something like 'faded blue bags contain no other bags.')
        const color = split.slice(1, split.length - 1).join(' ');                 // e.g. 'bright white' (we got rid of the number and the "bag"/"bags")
        dict[container][color] = +split[0];                                       // e.g. dict['light red bags']['bright white bag'] = 1;
      }
    }
  }

  if (part === 1) {                                                               // PART 1: COUNT HOW MANY BAGS ULTIMATELY CONTAIN 'shiny gold'

    // OPTION A: use a stack, and search all bags that contain the current target (which starts as 'shiny gold'). when found, make that bag a new target in the stack.

    // const output = new Set();                                                     // prevent duplicates
    // const processed = new Set();                                                  // prevent repeated work
    // const stack = ['shiny gold'];                                                 // begin our search with 'shiny gold' as the target
    // while (stack.length) {
    //   const target = stack.pop();                                                 // target is the color of bag we're searching for within all other bag colors
    //   if (processed.has(target)) continue;
    //   processed.add(target);
    //   for (const container of Object.keys(dict)) {                                // search all bag colors for the target
    //     if (target in dict[container]) {                                          // if we find a bag that contains the target...
    //       output.add(container);                                                  // ...then this bag ultimately contains 'shiny gold'
    //       stack.push(container);                                                  // ...and next, we will search for all bags that contain this one
    //     }
    //   }
    // }

    // return output.size;                                                           // the size of the output set is the number of bags that ultimately contain 'shiny gold'

    // OPTION B: create a memoization dictionary of bags with booleans for whether they ultimately contain 'shiny gold'. to populate, check against dict and see if that color contains
    // 'shiny gold' (or contains another color that does) via recursive DFS

    const containsShinyGold = {};                                                 // memoization dictionary
    function DFS(container) {
      if (!(container in containsShinyGold)) {                                    // calculate if not already memoized
        const contents = Object.keys(dict[container]);
        containsShinyGold[container] = contents.length && (                       // if this bag color does not contain anything, then return false
          'shiny gold' in dict[container] ||                                      // if this bag color contains 'shiny gold', then return true
          contents.some(content => DFS(content))                                  // if this bag color only contains other colors, check each of those and return true on the first hit
        );
      }
      return containsShinyGold[container];
    }
    let count = 0;
    for (const color of Object.keys(dict)) {                                      // search all bag colors for 'shiny gold'
      if (DFS(color)) ++count;                                                    // if that color contains 'shiny gold' then increment the count
    }
    return count;

  } else {                                                                        // PART 2: COUNT HOW MANY BAGS ARE INSIDE ONE "shiny gold" BAG

    function DFS(color) {
      let count = 0;
      for (const content of Object.keys(dict[color])) {                           // for each type of bag that must be inside the input bag...
        count += dict[color][content] * (1 + DFS(content));                       // ...increase count by the number of the inner bags as well as their contents (found recursively)
      }
      return count;                                                               // return the number of bags that must be inside of 1 bag of the input color
    }

    return DFS('shiny gold');                                                     // kick start the DFS with 'shiny gold' to see how many bags must be inside 1 shiny gold bag
