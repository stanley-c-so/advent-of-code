...
function countYes (part, inputStr) {
  const inputArr = inputStr.split('\n\n');                                // split the groups by the occurrence of two newlines in a row

  let yes = 0;

  for (let group of inputArr) {
    const responsesByPerson = group.split('\n');
    const numOfPeople = responsesByPerson.length;
    const responseFreq = {};
    for (const char of responsesByPerson.join('')) {
      responseFreq[char] = responseFreq[char] + 1 || 1;
    }
    const questions = Object.keys(responseFreq);
    yes += (part === 1
      ? questions.length                                                  // PART 1: COUNT HOW MANY QUESTIONS WERE ANSWERED "YES" BY ANYONE
      : questions.filter(q => responseFreq[q] === numOfPeople).length     // PART 2: COUNT HOW MANY QUESTIONS WERE ANSWERED "YES" BY EVERYONE
    );
  }
...
const actualInput = `elmcifawqgkp
dynwslejfcrkpv
